...
void ComputeParticleForce(){
    ...
    for (int j=0; j<voxel_block_size; ++j){
        ...
        if (index_j>0){
            ...
            if (rij<h){
                ...
                a_viscosity -= particle_volume * (Particle[particle_index-1+off][1].xyz-Particle[index_j-1+off][1].xyz) * 2 * length(kernel_tmp)/(rij);  // div()
                // curl
                curl += particle_volume * cross(Particle[index_j-1+off][1].xyz, kernel_tmp);
                // div wss
                div_wss += particle_volume * dot(Particle[particle_index-1+off][1].xyz, kernel_tmp);
    ...
    Particle[particle_index-1+off][3].xyz = a_viscosity;
    ParticleSubData[particle_index-1+off][3].z = length(curl);
    ParticleSubData[particle_index-1+off][1].xyz = curl;
    Particle[particle_index-1+off][2].w = div_wss;
}
