...
void ComputeParticleDensityPressure(){
    ...
    vec3 particle_pos = Particle[particle_index-1+off][0].xyz;
    // delete its density and pressure last time, optional
    //ParticleSubData[particle_index-1][2].w = Particle[particle_index-1][2].w;
    Particle[particle_index-1+off][2].w = 0.0;
    Particle[particle_index-1+off][3].w = 0.0;
    ...
    for (int j=0; j<voxel_block_size; ++j){
        ...
        // P_j is a domain particle
            ...
            // distance rij
            float rij = distance(particle_pos, Particle[index_j-1+off][0].xyz);
            // distance less than h
            if (rij<h){
                kernel_tmp = wendland_3d(rij, h);
                // add density to location (2, 2) of its mat4x4
                //     P_i_rho       +=         P_j_mass       * poly6_3d(rij, h)
                Particle[particle_index-1+off][2].w += Particle[index_j-1+off][1].w * kernel_tmp * particle_volume;
    ...
    Particle[particle_index-1+off][3].w = Particle[particle_index-1+off][1].w;
    ParticleSubData[particle_index-1+off][2].x = particle_volume*kernel_value;
}
