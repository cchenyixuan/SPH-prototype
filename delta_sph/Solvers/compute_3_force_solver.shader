    ...
    for (int j=0; j<voxel_block_size; ++j){
        ...
        if (index_j>0){
            ...
            if (rij<h){
                ...
                a_viscosity += viscosity * h * c0 * rest_dense * (Particle[particle_index-1][1].xyz-Particle[index_j-1][1].xyz)*(xij)/rij/rij * kernel_tmp * Particle[index_j-1][1].w/Particle[index_j-1][2].w;
        ...
        // P_j is a boundary particle
            ...
            if (rij<h){
                ...
                // a_visco  += VISC * 2*(dimension+2) * MASS_j/P_j_rho * (P_i_v - P_j_v)*(P_i_x-P_j_x)/(rij*rij + 0.01*H**2) * grad_spiky_3d(xij, rij, H)
                a_viscosity += viscosity * h * c0 * rest_dense * (Particle[particle_index-1][1].xyz-BoundaryParticle[index_j-1][1].xyz)*(xij)/rij/rij * kernel_tmp * BoundaryParticle[index_j-1][1].w/BoundaryParticle[index_j-1][2].w;
