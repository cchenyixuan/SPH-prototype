...
const float particle_volume = 8.889929493801344e-05;
...
void main() {
    ...
    if(Particle[v_index][0].w != 0.0){
        ...
        g_out.vv_pos = vec4(Particle[v_index][0].x, Particle[v_index][3].x, Particle[v_index][0].z, 1.0);
        g_out.vv_color = vec4(Particle[v_index][3].x/delta_t, -Particle[v_index][3].x/delta_t, 0.0, 1.0);
        g_out.vvv_color = Particle[v_index][1].x==0.0 ? vec4(0.0): vec4(abs(sin(Particle[v_index][1].x)), abs(cos(Particle[v_index][1].x)), abs(sin(2*Particle[v_index][1].x)), 1.0);
        switch(color_type){
            case 0:  // n  0
                v_color = vec4(abs(sin(Particle[v_index][1].x)), abs(cos(Particle[v_index][1].x)), abs(sin(2*Particle[v_index][1].x)), 1.0);
                // gl_Position = projection*view*vec4(Particle[v_index][0].x, Particle[v_index][2].z, Particle[v_index][0].z, 1.0);
                g_out.v_color = v_color;
                g_out.v_pos = vec4(Particle[v_index][0].x, Particle[v_index][1].x, Particle[v_index][0].z, 1.0);
                break;
            case 1:  // dn/dt, red+green-  1
                v_color = vec4(Particle[v_index][3].x/delta_t, -Particle[v_index][3].x/delta_t, 0.0, 1.0);
                // gl_Position = projection*view*vec4(Particle[v_index][0].x, Particle[v_index][2].z, Particle[v_index][0].z, 1.0);
                g_out.v_color = v_color;
                g_out.v_pos = vec4(Particle[v_index][0].x, Particle[v_index][1].x, Particle[v_index][0].z, 1.0);
                break;
            case 2:  // F1  2
                v_color = vec4(Particle[v_index][1].z/delta_t, -Particle[v_index][1].z/delta_t, 0.0, 1.0);
                // gl_Position = projection*view*vec4(Particle[v_index][0].x, length(Particle[v_index][1].xy), Particle[v_index][0].z, 1.0);
                g_out.v_color = v_color;
                g_out.v_pos = vec4(Particle[v_index][0].x, Particle[v_index][1].z/delta_t, Particle[v_index][0].z, 1.0);
                break;
            case 3:  // lap n  3
                v_color = vec4(Particle[v_index][1].y, -Particle[v_index][1].y, 0.0, 1.0);
                // gl_Position = projection*view*vec4(Particle[v_index][0].x, Particle[v_index][2].x, Particle[v_index][0].z, 1.0);
                g_out.v_color = v_color;
                g_out.v_pos = vec4(Particle[v_index][0].x, Particle[v_index][1].y, Particle[v_index][0].z, 1.0);
                break;
            case 4:  // F2  4
                v_color = vec4(Particle[v_index][2].z/delta_t, -Particle[v_index][2].z/delta_t, 0.0, 1.0);
                // gl_Position = projection*view*vec4(Particle[v_index][0].x, length(Particle[v_index][1].xy), Particle[v_index][0].z, 1.0);
                g_out.v_color = v_color;
                g_out.v_pos = vec4(Particle[v_index][0].x, Particle[v_index][2].z/delta_t, Particle[v_index][0].z, 1.0);
                break;
            case 5:  // lap n2  5
