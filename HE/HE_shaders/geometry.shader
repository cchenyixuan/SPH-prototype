...
void main() {
    ...
    // 2d-version
    v_color = g_in[0].v_color;
    vec4 v_pos2 = vec4(g_in[0].v_pos.x+25, g_in[0].v_pos.z, 0.0, 1.0);
    gl_Position = projection*view*v_pos2;
    EmitVertex();
    EndPrimitive();
    // dv/dt
    v_color = g_in[0].vv_color;
    vec4 v_pos3 = vec4(g_in[0].vv_pos.x-25, g_in[0].vv_pos.yzw);
    gl_Position = projection*view*v_pos3;
    EmitVertex();
    EndPrimitive();
    // dv/dt 2d version
    v_color = g_in[0].vv_color;
    vec4 v_pos4 = vec4(g_in[0].vv_pos.x-50, g_in[0].vv_pos.z, 0.0, 1.0);
    gl_Position = projection*view*v_pos4;
    EmitVertex();
    EndPrimitive();
    // u 2d version colored by |u|
    v_color = g_in[0].vvv_color;
    vec4 v_pos5 = vec4(g_in[0].v_pos.x+50, g_in[0].v_pos.z, 0.0, 1.0);
