...
float wendland_2d(float rij, float h){
    float q = rij/h;
    if (q > 1.0){return 0.0;}
    return Coeff_Wendland_2d * pow(1.0-q, 6.0)*(35.0/3.0*q*q+6.0*q+1.0);
}
vec2 grad_wendland_2d(float x, float y, float rij, float h){
    float q = rij/h;
    if (q > 1.0){return vec2(0.0);}
    float w_prime = Coeff_Wendland_2d / h * (-56.0/3.0) * q * (1.0+5.0*q) * pow(1.0-q, 5.0);
...
void ComputeParticleProperties(){
    ...
    Particle[particle_index-1][1].yzw = vec3(0.0);
    Particle[particle_index-1][2] = vec4(0.0);
    ...
    for (int j=0; j<voxel_block_size; ++j){
        ...
        // P_j is a domain particle
            ...
            if (particle_index == index_j){continue;}
                ...
                // lap u
                Particle[particle_index-1][1].y -= particle_volume * (Particle[particle_index-1][1].x-Particle[index_j-1][1].x) * 2.0 * length(kernel_tmp)/(rij);
                // Particle[particle_index-1][1].y += 8*particle_volume * (Particle[particle_index-1][1].x-Particle[index_j-1][1].x) * dot(xij, kernel_tmp)/(rij*rij + 0.01*h2);
            }
        }
    ...
    // L^-1
    Particle[particle_index-1][1].y *= 1.0;//1.03054677*1.115;
    // F1
    Particle[particle_index-1][1].z = delta_t*Particle[particle_index-1][1].y;
    // u + F1
    Particle[particle_index-1][2].x = Particle[particle_index-1][1].x + delta_t*Particle[particle_index-1][1].y;
