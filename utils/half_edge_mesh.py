    ...
    def delete_facet(self, facet: HalfEdgeFacet):
        if facet.index not in self.half_edge_facets.keys():
            ...
            for key in sorted(facet.half_edge.keys(), reverse=True):
                ...
                # half_edge.next.next.next = None
                half_edge.vertex.half_edge.pop(half_edge.index)
                # switch half-edge
                self.half_edges[half_edge.index], self.half_edges[self.half_edge_max_index] = self.half_edges[self.half_edge_max_index], self.half_edges[half_edge.index]
                # fix vertex and facet index
                self.half_edges[half_edge.index].facet.half_edge.pop(self.half_edge_max_index)
                self.half_edges[half_edge.index].facet.half_edge[half_edge.index] = self.half_edges[half_edge.index]
                self.half_edges[half_edge.index].vertex.half_edge.pop(self.half_edge_max_index)
                self.half_edges[half_edge.index].vertex.half_edge[half_edge.index] = self.half_edges[half_edge.index]
                self.half_edges[half_edge.index].index = half_edge.index
                self.half_edges.pop(self.half_edge_max_index)
...
#
            ...
            # remove facet
            self.half_edge_facets[facet.index], self.half_edge_facets[self.facet_max_index] = self.half_edge_facets[self.facet_max_index], self.half_edge_facets[facet.index]
            self.half_edge_facets[facet.index].index = facet.index
            self.half_edge_facets.pop(self.facet_max_index)
    ...
    @staticmethod
    def save_stl(file_name, vertices, facets):
        with open(file_name, "w") as f:
            f.write("solid Exported from HalfEdge\n")
            for facet in facets:
                v0 = vertices[facet[0] - 1]
                v1 = vertices[facet[1] - 1]
                v2 = vertices[facet[2] - 1]
                tmp = np.cross(v1 - v0, v2 - v1)
                n = tmp / np.linalg.norm(tmp)
                f.write(
                    f"facet normal {n[0]} {n[1]} {n[2]}\nouter loop\nvertex {v0[0]} {v0[1]} {v0[2]}\nvertex {v1[0]} {v1[1]} {v1[2]}\nvertex {v2[0]} {v2[1]} {v2[2]}\nendloop\nendfacet\n")
            f.write("endsolid Exported from HalfEdge\n")
        f.close()
        print("Done export stl.")

    @staticmethod
    def save_obj(file_name, vertices, facets):
        with open(file_name, "w") as f:
            ...
            f.write("vn 0 0 0\n")
            for facet in facets:
                f.write(f"f {facet[0]}/1/1 {facet[1]}/1/1 {facet[2]}/1/1\n")
        f.close()
        print("Done export obj.")
    ...
    def extend(self, distance: float, reverse: bool = False) -> None:
        """
        Extend mesh along facet normal by a certain distance.
        :param distance:
        :param reverse:
        :return:
        """
        # extend all vertices;
        # if two vertices move towards each-other, and overlap occurs, these two vertices should be merged at center.
        # overlap is checked by the direction of p1-p2, if (p1-p2)@(p1'-p2')<=0.
        # shift part
        # for all vertices
        new_half_edge_vertices = dict()
        for key in self.half_edge_vertices.keys():
            vertex = self.half_edge_vertices[key]
            # calculate vertex normal vector by the average of facet normals
            vertex_normal = np.zeros((3, ), dtype=np.float32)
            for half_edge in vertex.half_edge.values():
                vertex_normal += half_edge.facet.cal_normal()
            vertex_normal /= np.linalg.norm(vertex_normal)  # normal vector with length 1
            # extend along normal by a certain distance
            new_half_edge_vertices[key] = HalfEdgeVertex(*(vertex.numpy+vertex_normal*distance), key) if not reverse else HalfEdgeVertex(*(vertex.numpy-vertex_normal*distance), key)
            new_half_edge_vertices[key].half_edge = dict(vertex.half_edge)
        # check if any half-edge reverses, if so, mark it
        key = 0
        while True:
            if key > self.half_edge_max_index:
                break
            half_edge = self.half_edges[key]
            if half_edge.index is None:
                continue
            v1 = half_edge.vertex
            v2 = half_edge.next.vertex
            if v1.index == v2.index:
                key += 1
                continue
            new_v1 = new_half_edge_vertices[v1.index]
            new_v2 = new_half_edge_vertices[v2.index]
            if (v1.numpy - v2.numpy) @ (new_v1.numpy - new_v2.numpy) <= 0:  # overlap
                # shift new_v1 to center
                new_v1.numpy = 0.5*(new_v1.numpy+new_v2.numpy)
                new_v1.x, new_v1.y, new_v1.z = new_v1.numpy
                # remove the half-edge
                self.delete_edge(half_edge)
                # shift all half-edges from p2 and shift all half-edges towards p2
                print(v2.half_edge)
                while v2.half_edge:
                    _, item = v2.half_edge.popitem()
                    item.vertex = v1
                    v1.half_edge[item.index] = item
                # remove v2
                print(v2.half_edge)
                new_half_edge_vertices[v2.index], new_half_edge_vertices[self.vertex_max_index] = new_half_edge_vertices[self.vertex_max_index], new_half_edge_vertices[v2.index]

                new_half_edge_vertices[v2.index].index = v2.index
                new_half_edge_vertices.pop(self.vertex_max_index)
                self.half_edge_vertices[v2.index], self.half_edge_vertices[self.vertex_max_index] = self.half_edge_vertices[self.vertex_max_index], self.half_edge_vertices[v2.index]

                self.half_edge_vertices[v2.index].index = v2.index
                self.half_edge_vertices.pop(self.vertex_max_index)
                self.vertex_max_index -= 1
            else:
                key += 1
        # upgrade changes
        self.half_edge_vertices = new_half_edge_vertices


if __name__ == "__main__":
    geometry = r"untitled.obj"
    ...
    #     tmp = []
    #     for v in hf.half_edge_vertices_rev.keys():
    #         if len(v.half_edge) == 0:
    #             tmp.append(v)
    #     # for vertex in tmp:
