...
void AllocateParticles(){
    ...
    vec3 particle_pos;
    if(particle_index-1<StatusInt[3]){
        //inlet1
        particle_pos = Inlet1[particle_index-1][0].xyz;
    }
    else if(particle_index-1<StatusInt[3]+StatusInt[4]){
        //inlet2
        particle_pos = Inlet2[particle_index-1-StatusInt[3]][0].xyz;
    }
    else if(particle_index-1<StatusInt[3]+StatusInt[4]+StatusInt[5]){
        //inlet3
        particle_pos = Inlet3[particle_index-1-StatusInt[3]-StatusInt[4]][0].xyz;
    }
    // for all voxels
    for(int i=0; i < n_voxel; ++i){
        ...
        if(
            ...
            voxel_pos.z-h/2<=particle_pos.z && particle_pos.z<voxel_pos.z+h/2
                ...
                if(particle_index-1<StatusInt[3]){
                    //inlet1
                    Inlet1[particle_index-1][0].w = float(i+1);  // starts from 1.0
                }
                else if(particle_index-1<StatusInt[3]+StatusInt[4]){
                    //inlet2
                    Inlet2[particle_index-1-StatusInt[3]][0].w = float(i+1);  // starts from 1.0
                }
                else if(particle_index-1<StatusInt[3]+StatusInt[4]+StatusInt[5]){
                    //inlet3
                    Inlet3[particle_index-1-StatusInt[3]-StatusInt[4]][0].w = float(i+1);  // starts from 1.0
                }
                break;
        };
