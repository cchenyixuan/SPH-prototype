    ...
    for (int j=0; j<voxel_block_size; ++j){
        ...
        if (index_j>0){
            ...
            if (rij<h){
                ...
                // Heat Equation with Brookshaw's discrete Laplacian
                heat_transfer -= Particle[index_j-1][1].w/Particle[index_j-1][2].w * (Particle[particle_index-1][2].z - Particle[index_j-1][2].z) * 2*length(kernel_tmp)/rij;

                //xsph
                xsph_velocity -= Particle[index_j-1][1].w/Particle[index_j-1][2].w*(Particle[particle_index-1][1].xyz-Particle[index_j-1][1].xyz)*wendland_3d(rij, h);
            }
        }
